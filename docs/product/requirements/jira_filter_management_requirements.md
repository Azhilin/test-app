...
| JFM-D-002 | The default filter pre-sets sensible parameter defaults | The default entry has `JIRA_FILTER_STATUS=Done`, `JIRA_CLOSED_SPRINTS_ONLY=true`, and `schema_name=Default_Jira_Cloud`; `JIRA_PROJECT` is intentionally blank (instance-specific) | ✓ Met | — |
...
| JFM-P-006 | `POST /api/filters` builds correct JQL from params | The generated JQL includes `project =`/`IN`, optional `status =`/`IN`, optional `Team[Team] =`/`IN`, optional `type IN`, and `sprint in closedSprints()` when `JIRA_CLOSED_SPRINTS_ONLY` is truthy | ✓ Met | — |
